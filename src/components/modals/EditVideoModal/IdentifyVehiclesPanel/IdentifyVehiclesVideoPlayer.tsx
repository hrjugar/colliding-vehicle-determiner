...
interface IdentifyVehiclesVideoProps {
  // Add any props you need for the component
}

const IdentifyVehiclesVideoPlayer: React.FC<IdentifyVehiclesVideoProps> = () => {
  const [
    getSelectedObject,
    duration,
    setDuration,
  ...
  return (
    <div className='card w-full h-full flex flex-col'>
      ...
      <div className='w-full h-10 px-4 flex justify-center items-center gap-4'>
        ...
        <div className='relative bg-gray-200 w-full h-2 rounded-full flex flex-row items-center'>
          <div 
            className='absolute h-2 rounded-full bg-color-primary-active/50'
            style={{ width: `${timePercentage}%` }}
          />
          <div 
            className='absolute w-4 h-4 rounded-full bg-color-primary -translate-x-1/2'
